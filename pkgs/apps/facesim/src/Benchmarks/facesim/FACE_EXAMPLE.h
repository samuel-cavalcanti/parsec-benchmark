...
namespace PhysBAM
{
	...
	public:
		...
		void Write_Output_Files(const int frame) const
		{
			...
			if (optimization)
				optimization->template Write_Optimization_Data<RW>(output_directory + "/", frame);
			else
		...
		void Read_Output_Files_Solids(const int frame)
		{
			...
			if (optimization)
				optimization->template Read_Optimization_Data<RW>(output_directory + "/", frame);
			else
		...
		void Initialize_Bodies()
		{
			...
			for (int i = 1; i <= number_of_muscles; i++)
				activation_controls->Add_Activation();
			...
			if (solids_parameters.perform_collision_body_collisions)
			{
				...
				if (optimization)
					optimization->solids_evolution_callbacks = this;
			}
		}
		//#####################################################################
		...
		void Get_Initial_Data()
		{
			...
			for (int i = 1; i <= number_of_muscles; i++)
			{
				...
				if (verbose)
					std::cout << "Reading muscle data : " << muscle_names(i) << ".constitutive_data" << std::endl;
			...
			for (int i = 1; i <= peak_isometric_stress_entries; i++)
			{
				...
				if (muscle_names.Find(muscle_name, index))
					peak_isometric_stress(index) = peak_isometric_stress_value;
			}
			...
			if (verbose)
				for (int i = 1; i <= number_of_muscles; i++)
					std::cout << muscle_names(i) << " : Peak isometric stress = " << peak_isometric_stress(i) << std::endl;

			for (int i = 1; i <= peak_isometric_stress.m; i++)
				peak_isometric_stress(i) *= peak_isometric_stress_boosting;
		...
		//#####################################################################
		template <class T2>
		struct ZERO_OUT_ENSLAVED_POSITION_NODES_HELPER
		{
		...
		static void Zero_Out_Enslaved_Position_Nodes_Helper(long thread_id, void *helper_raw)
		{
			...
			for (int i = 1; i <= partition_attached_nodes.m; i++)
				X(partition_attached_nodes(i)) = VECTOR_3D<T>();
		}
		void Zero_Out_Enslaved_Position_Nodes(ARRAY<VECTOR_3D<T>> &X, const T time, const int id_number)
		{
			...
			case 1:
				...
				if (PHYSBAM_THREADED_RUN)
				{
					...
					for (int i = 1; i <= attached_nodes.m; i++)
						for (int j = 1; j <= attached_nodes(i).m; j++)
							X(attached_nodes(i)(j)) = VECTOR_3D<T>();

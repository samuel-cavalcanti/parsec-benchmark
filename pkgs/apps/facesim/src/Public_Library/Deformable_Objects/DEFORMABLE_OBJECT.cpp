...
#else
template <class T, class TV>
struct CONJUGATE_GRADIENTS_HELPER
{
										...
										int *iterations_used, const bool update_positions_and_state)
{
	...
	if (update_positions_and_state)
		external_forces_and_velocities->Set_External_Positions(particles.X.array, time, id_number);
	...
	for (iterations = 1; iterations <= max_iterations; iterations++)
	{
		...
		else
		{
			...
			for (i = 1; i <= N; i++)
				S_dot_Q -= TV::Dot_Product(S_full(i), negative_Q_full(i));
	...
	if (iterations_used)
		*iterations_used = min(iterations, max_iterations);
	...
	Update_Position_Based_State()
{
	...
	for (int k = 1; k <= solids_forces.m; k++)
		if (solids_forces(k)->use_position_based_state)
			solids_forces(k)->Update_Position_Based_State();
	...
	Delete_Position_Based_State()
{
	for (int k = 1; k <= solids_forces.m; k++)
		if (solids_forces(k)->use_position_based_state)
			solids_forces(k)->Delete_Position_Based_State();
}
//#####################################################################
	...
	Add_Velocity_Independent_Forces(ARRAY<TV> &F) const
{
	...
	for (int k = 1; k <= solids_forces.m; k++)
		if (solids_forces(k)->use_velocity_independent_forces)
			solids_forces(k)->Add_Velocity_Independent_Forces(F);
	...
	Add_Velocity_Dependent_Forces(ARRAY<TV> &F) const
{
	...
	for (int k = 1; k <= solids_forces.m; k++)
		if (solids_forces(k)->use_velocity_dependent_forces)
			solids_forces(k)->Add_Velocity_Dependent_Forces(F);
	...
	Add_Velocity_Dependent_Forces(ARRAY<TV> &F, const int partition_id) const
{
	for (int k = 1; k <= solids_forces.m; k++)
		if (solids_forces(k)->use_velocity_dependent_forces)
			solids_forces(k)->Add_Velocity_Dependent_Forces(F, partition_id);
}
//#####################################################################
	...
	Force_Differential(const ARRAY<TV> &dX, ARRAY<TV> &dF) const
{
	...
	// LOG::Stop_Time();
	for (int k = 1; k <= solids_forces.m; k++)
		solids_forces(k)->Add_Force_Differential(dX, dF);
	...
	Force_Differential(const ARRAY<TV> &dX, ARRAY<TV> &dF, const int partition_id) const
{
	...
	for (int k = 1; k <= solids_forces.m; k++)
		solids_forces(k)->Add_Force_Differential(dX, dF, partition_id);
}
//#####################################################################
...
{
	for (int i = 1; i <= solids_forces.m; i++)
		solids_forces(i)->Enforce_Definiteness(enforce_definiteness_input);
}
//#####################################################################
// Function CFL
//#####################################################################
template <class T, class TV>
T DEFORMABLE_OBJECT<T, TV>::
	...
	CFL_Elastic_And_Damping()
{
	...
	if (!implicit_damping)
		dt_damping = CFL_Damping();

	T one_over_dt_full = (1 / dt_elastic + 1 / dt_damping) / cfl_number;

	if (one_over_dt_full > 1 / FLT_MAX)
		return 1 / one_over_dt_full;
	else
		return FLT_MAX;
}
//#####################################################################
	...
	CFL_Elastic()
{
	...
	for (int k = 1; k <= solids_forces.m; k++)
		if (solids_forces(k)->use_velocity_independent_forces)
			hertz += 1 / solids_forces(k)->CFL_Velocity_Independent();
	...
	CFL_Damping()
{
	...
	for (int k = 1; k <= solids_forces.m; k++)
		if (solids_forces(k)->use_velocity_dependent_forces)
			hertz += 1 / solids_forces(k)->CFL_Velocity_Dependent();
	...
	CFL_Strain_Rate()
{
	...
	for (int k = 1; k <= solids_forces.m; k++)
		if (solids_forces(k)->limit_time_step_by_strain_rate)
			hertz = max(hertz, 1 / solids_forces(k)->CFL_Strain_Rate()); // otherwise not included
